...
def test_predictive_models_router_isolated_error_contract(tmp_path: Path) -> None:
    app = FastAPI()
    register_domain_exception_handlers(app)
    app.include_router(predictive_models_router)

    client = TestClient(app)
    response = client.get("/api/predictive-models/nonexistent_model")
    assert response.status_code == 404
    data = response.json()
    assert data["code"] == "predictive_model_not_found"
...
def test_valuations_router_isolated_error_contract(tmp_path: Path) -> None:
    store = ProjectStore(tmp_path)
    app = FastAPI()
    register_domain_exception_handlers(app)
    app.include_router(valuations_router)
    ...
    client = TestClient(app)
    response = client.get(f"/api/projects/{uuid4()}/valuations")
    assert response.status_code == 404
    data = response.json()
    assert data["code"] == "project_not_found"
